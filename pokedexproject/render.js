...
function renderFilteredPokemon(filterdArray, filterIndex){
    return `
...
function renderOverlayDiv(i){
    ...
    <h3>${allPokemon[i].name}</h3><img onclick="skipToNext(${i})"; class="skipToNext" id="skiptonext" src="./images/icons/skipToNext.png"></div><img onclick="toggleOverlayWrapper(${i})" class="CloseButton" src="./images/img/close_button.png">
    </div>
...
function renderMainData(mainDataIndex, i, mainDataArrayWord, value3, value4){
return `
    <table>
    <tr>
        <td class="leftLine"><h3>${mainDataIndex === 4 ? "" : mainDataArrayWord[mainDataIndex]+ ":"}</h3></td>
        <td class="rightLine"><h3>${mainDataIndex === 3 ? value3  +  value4 : mainDataIndex === 4 ? "" : allPokemon[i][mainDataArrayWord[mainDataIndex]]}</h3></td>
    </tr>
    </table>
    `
}
...
function renderStats(i, statsIndex, textX, textY){
    return `<div>
    <table>
    <tr>
        <td><h3>${allPokemon[i].stats[statsIndex].stat.name}</h3></td>
